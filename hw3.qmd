...
The variables selected are Intercept, volatile_acidity, citric_acid, residual_sugar, chlorides, total_sulfur_dioxide, density, pH, sulphates, alcohol, and typewhite.
---


###### 3.6  (10 points)

What is the difference between step wise selection, LASSO and ridge based on your analyses above?

The ridge based version is least aggressive. Coefficients only approach zero while the other two can be zero. Lasso is the most aggressive in variable selection making it better for when the number of columns is high.
...
```{R}
total_models <- sum(choose(10, 0:10))
print(total_models)
```
...
formulas <- map(
  ...
  \(x){
    ...
    map(vars, ~ as.formula(paste("quality ~", paste(.x, collapse = "+")))) 
  }
) %>% unlist()
```
...
They are not the same and they are different because aic uses lowest AIC value to select and rsq uses highest r squared.

* Which of these is more reliable? Why? 
AIC is because it compares the goodness of the fit with the model and the data.

* If we had a dataset with $10,000$ columns, which of these methods would you consider for your analyses? Why?
I would use LASSO in the case of high dimensionality becuase it would select in a more agressive way.
---
...
```R
# Function to fit the model correctly based on the presence of the intercept term
fit_model <- function(formula, data) {
  if ("(Intercept)" %in% all.vars(formula)) {
    lm(formula, data = data)
  } else {
    lm(formula, data = data, intercept = FALSE)
  }
}
